  ...
  class ISConfig
  {
    ...
    public static string GenerateAGSToken_RESTAdmin()
    {
      ...
      try
      {
        ...
        if (result.IndexOf("error", StringComparison.CurrentCultureIgnoreCase) >= 0)
        {
    ...
    public static void ListServices_RESTAdmin(string root, string folder)
    {
      try
      {
        ...
        while (index > 0)
        {
          ...
          {
            Console.WriteLine("Error looking at service: {0}", exc.Message);
          }
        }
      }
      catch (Exception exc)
      {
        Console.WriteLine("Error looking at services: {0}", exc.Message);
      }
    }
    /// <summary>
    ...
    /// <param name="createImageServiceParams">Creation parameters, e.g. raster functions, colormaptorgb, raster types, dem, dynamicimageworkspace, customized propertyset etc</param>
